...
ROR_file = r"D:\2022-2023\thesis\bottomup\0911\data\individual_instr\indivudual_trendcarry\missingdatahandling\OF\Otherfunds_missing\SGCTA.csv"

# Read CSV file
data = pd.read_csv(csv_file, parse_dates=['Date'], date_parser=lambda x: pd.to_datetime(x, format='%m/%d/%Y'))
ROR = pd.read_csv(ROR_file, parse_dates=['Date'], date_parser=lambda x: pd.to_datetime(x, format='%m/%d/%Y'))
...
csv_file = r"D:\2022-2023\thesis\bottomup\0911\data\individual_instr\indivudual_trendcarry\missingdatahandling\OF\results\SGCTA.csv"
...
plot_data = pd.DataFrame({'Cumulative_Return_Predicted': cumulative_return, 'Cumulative_Return_SGCTA': AB}, index=ROR.index)


fig, ax = plt.subplots()
plt.plot(plot_data.index, plot_data['Cumulative_Return_Predicted'], label='Cumulative Return Predicted')
plt.plot(plot_data.index, plot_data['Cumulative_Return_SGCTA'], label='Cumulative Return SGCTA')


plt.xlabel('Year')
plt.ylabel('Cumulative Return')
plt.title('Cumulative Return SG CTA and the Bottom-up Method')
plt.legend()
